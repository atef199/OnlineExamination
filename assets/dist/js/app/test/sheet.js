...
function cek_status_ragu(q_id) {
    var status_ragu = $("#rg_" + q_id).val();
...
function cek_terakhir(q_id) {
    var jml_question = $("#jml_question").val();
    jml_question = (parseInt(jml_question) - 1);

    if (jml_question === q_id) {
        $('.next').hide();
        $(".selesai, .back").show();
    } else {
        $('.next').show();
...
function simpan_sementara() {
    ...
    var jml_question = form.jml_question;
    jml_question = parseInt(jml_question);

    var hasil_right_ans = "";

    for (var i = 1; i < jml_question; i++) {
        var idx = 'ans_' + i;
        var idx2 = 'rg_' + i;
        var jawab = form[idx];
        var ragu = form[idx2];
        ...
        if (jawab != undefined) {
            if (ragu == "Y") {
                if (jawab == "-") {
                    hasil_right_ans += '<a id="btn_question_' + (i) + '" class="btn btn-default btn_question btn-sm" onclick="return buka(' + (i) + ');">' + (i) + ". " + jawab + "</a>";
                } else {
                    hasil_right_ans += '<a id="btn_question_' + (i) + '" class="btn btn-warning btn_question btn-sm" onclick="return buka(' + (i) + ');">' + (i) + ". " + jawab + "</a>";
                }
            } else {
                if (jawab == "-") {
                    hasil_right_ans += '<a id="btn_question_' + (i) + '" class="btn btn-default btn_question btn-sm" onclick="return buka(' + (i) + ');">' + (i) + ". " + jawab + "</a>";
                } else {
                    hasil_right_ans += '<a id="btn_question_' + (i) + '" class="btn btn-success btn_question btn-sm" onclick="return buka(' + (i) + ');">' + (i) + ". " + jawab + "</a>";
                }
            }
        } else {
            hasil_right_ans += '<a id="btn_question_' + (i) + '" class="btn btn-default btn_question btn-sm" onclick="return buka(' + (i) + ');">' + (i) + ". -</a>";
        }
    }
    $("#tampil_right_ans").html('<div id="yes"></div>' + hasil_right_ans);
}
