...
$mulai = strftime('%A, %d %B %Y', strtotime($test->start_date));
$selesai = strftime('%A, %d %B %Y', strtotime($test->end_date));
...
$html .= <<<EOD
    <tr>
        ...
        <td width="15%">{$row->level_name}</td>
        <td width="25%">{$row->dep_name}</td>
        <td width="10%">{$row->correct_ans_num}</td>
        <td width="10%">{$row->mark}</td>
    </tr>
EOD;
$no++;
}
